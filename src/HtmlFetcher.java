...
public class HtmlFetcher {
    ...
    public static String fetch(URL url, int redirects) {
        try {
            ...
            while(isRedirect(header) && redirects > 0) {
                String newURL = header.get("Location").get(0);
                header = HttpsFetcher.fetchURL(newURL);
                statusCode = getStatusCode(header);
                redirects--;
            }
            if (isHtml(header) && statusCode == 200) {
                return String.join(System.lineSeparator(), header.get("Content"));
            }
        } catch (IOException e) {
